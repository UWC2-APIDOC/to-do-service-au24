...
4. The response body should look something like this with the user name you supplied. The response should also include a new user `id`.
...
1. We'll use the `POST` command in our command-line tool to create a task for our new user to **get plane tickets from NYC to Portugal** before midnight on **11/01/24**. We will also send a reminder **24 hours** before the deadline.
    ...
    ```
2. The response should look something like this with a new task `id`.
    ```shell
    {
    "user_id": 7,
...
1. Add the `user_id` that was returned in the [first section](#enroll-a-new-user) of this tutorial to a cURL request to the `tasks` resource.
   ```shell
    ...
    ```
1. The response should return all the tasks you created in the [second section](#add-tasks) of this tutorial. It should look something like this.
    ```shell
    {
        "user_id": 7,
        "title": "Call Liza",
